...
if ($data) {
    ...
    if ($func) {
        ...
        // add item
        if ($func === "Add Item") {
            $itemName = htmlspecialchars($data['itemName']) ?? "Unnamed item";
            $catID = htmlspecialchars($data['catID']) ?? 0;
            $itemPrice = htmlspecialchars($data['itemPrice']) ?? 0;
            $itemStatus = htmlspecialchars($data['itemStatus']) ?? "active";
            $itemStock = htmlspecialchars($data['itemStock']) ?? 0;
            if (isset($data['itemDesc1'])) {
                $itemDesc1 = htmlspecialchars($data['itemDesc1']);
            } else {
                $itemDesc1 = "";
            }
            if (isset($data['itemDesc2'])) {
                $itemDesc2 = htmlspecialchars($data['itemDesc2']);
            } else {
                $itemDesc2 = "";
            }
            if (isset($data['itemDesc3'])) {
                $itemDesc3 = htmlspecialchars($data['itemDesc3']);
            } else {
                $itemDesc3 = "";
            }
