...
function EditItem(props) {
    ...
    // API get item data
    useEffect(() => {
        async function FN_apiGetItemData() {
            setFormLoading(true)
            await axios.post(props.apiURL, {func: 'Get Item', itemID: itemID})
                .then(results => FN_useItemData(results.data))
                .catch(err => {
                    setFormError(true)
    ...
    // use item data
    function FN_useItemData(data) {
        if (typeof data === 'undefined' || data === null) {
            setFormError(true)
            toast.error('Item could not be found')
        } else {
            setItemName(prev => ({...prev, name:data.itemName}))
            setCatID(prev => ({...prev, id:data.catID}))
            setItemPrice(prev => ({...prev, price:data.itemPrice}))
            setItemStatus(data.itemStatus)
            setItemStock(prev => ({...prev, stock:data.itemStock}))
            setItemDesc1(data.itemDesc1)
            setItemDesc2(data.itemDesc2)
            setItemDesc3(data.itemDesc3)
        }
    }
    ...
    async function FN_apiSubmit(e) {
        e.preventDefault()
        setFormLoading(true)
        await axios.post(props.apiURL, {func: 'Update Item', itemName: itemName.name, catID: catID.id, itemPrice: itemPrice.price, itemStatus: itemStatus, itemStock: itemStock.stock, itemDesc1: itemDesc1, itemDesc2: itemDesc2, itemDesc3: itemDesc3, itemID: itemID})
    ...
    return (
        <div className="popup_bg">
            <div className="popup_pnl">
                ...
                <div className="popup_ttl">Edit item</div>
