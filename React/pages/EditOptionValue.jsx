...
function EditOptionValue(props) {
    ...
    useEffect(() => {
        async function FN_apiGetOptValData() {
            setFormLoading(true)
            await axios.post(props.apiURL, {func: 'Get Option Value', valID: valID})
                .then(results => FN_useOptValData(results.data))
                .catch(err => {
                    setFormError(true)
    ...
    // use option data
    function FN_useOptValData(data) {
        if (typeof data === 'undefined' || data === null) {
            setFormError(true)
            toast.error('Option value could not be found')
        } else {
            setValName(prev => ({...prev, name:data.valName}))
        }
    }
    ...
    async function FN_apiSubmit(e) {
        e.preventDefault()
        setFormLoading(true)
        await axios.post(props.apiURL, {func: 'Update Option Value', valName: valName.name, valID: valID})
    ...
    return (
        <div className="popup_bg">
            <div className="popup_pnl">
                ...
                <div className="popup_ttl">Edit option value</div>
